...
def main():
    parser = argparse.ArgumentParser(
        description="Weld joint accessibility with +/- work-angle (up/down) tolerance rays")
    ...
    parser.add_argument("--tol_deg", type=float, default=10.0,
                         help="Work-angle tolerance swept up/down from the bisector, in degrees")
    parser.add_argument("--angle_step", type=float, default=None,
                         help="If set, fill the +/-tol_deg range with a ray every this many degrees "
                              "(e.g. tol_deg=10, angle_step=1 tests 21 rays/point) instead of just "
    ...
    analyze_and_visualize_with_tolerance(
        ...
        tol_deg=args.tol_deg, mode="workangle", angle_step=args.angle_step,
